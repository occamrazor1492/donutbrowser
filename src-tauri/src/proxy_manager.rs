...
// Extra/legacy fields in older JSON files are silently dropped on deserialization.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl ProxyManager {
  ...
  /// Resolve proxy settings for a specific profile.
  pub fn resolve_proxy_for_profile(&self, proxy_id: &str) -> Option<ProxySettings> {
    let stored_proxies = self.stored_proxies.lock().unwrap();
    stored_proxies
      .get(proxy_id)
